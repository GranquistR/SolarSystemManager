  ...
  <div class="flex justify-content-center">
    <div class="flex-column flex w-9">
      <Card class="mb-4">
        ...
        <template #content>
          <RouterLink to="/new">
            <Button style="font-weight: bold; font-size: 1.3em; color:ivory"> Create System </Button>
          </RouterLink>
        </template>
      </Card>
      <Card>
          ...
          <DataTable
            ...
            <Column header="Name">
              <template #body="slotProps">
                <span 
                v-tooltip.top="'Clicke to view and edit'"
                class="highlight-name">{{ slotProps.data.systemName }}</span>
              </template>
            </Column>
...
function processSolarSystems() {
  processedSolarSystems.value = solarSystems.value.map(solarSystem => {
    ...
    processedSystem.previewObjects = solarSystem.spaceObjects.length > 6
